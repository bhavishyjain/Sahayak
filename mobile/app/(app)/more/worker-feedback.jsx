    ...
    <View className="flex-row items-center" style={{ gap: 4 }}>
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          ...
          fill={star <= Math.round(value) ? colors.warning : "transparent"}
        />
      ))}
    </View>
  );
...
export default function WorkerFeedback() {
  ...
  const [summary, setSummary] = useState({ averageRating: 0, totalFeedback: 0 });
  ...
  const load = useCallback(async (isRefresh = false) => {
    try {
      ...
      setSummary(response?.data?.summary ?? { averageRating: 0, totalFeedback: 0 });
      setFeedback(response?.data?.feedback ?? []);
    } catch (error) {
      Toast.show({
        type: "error",
        text1: "Failed to load feedback",
  ...
  if (loading) {
    return (
      <View className="flex-1" style={{ backgroundColor: colors.backgroundPrimary }}>
        <BackButtonHeader title="Worker Feedback" />
          ...
          <View className="flex-row items-center justify-between">
            <View>
              ...
              <Text className="text-xs mt-1" style={{ color: colors.textSecondary }}>
                {summary.totalFeedback} feedback entr{summary.totalFeedback === 1 ? "y" : "ies"}
              </Text>
            </View>
          ...
          <Card style={{ margin: 0, flex: 0 }}>
            <View className="items-center py-8">
              ...
              <Text className="text-sm mt-1 text-center" style={{ color: colors.textSecondary }}>
                Resolved complaints with citizen ratings will appear here.
              </Text>
            </View>
          </Card>
        ) : (
          feedback.map((item) => (
            <Card key={item.complaintId} style={{ margin: 0, marginBottom: 12, flex: 0 }}>
              <View className="flex-row items-start justify-between mb-2">
                <View className="flex-1 pr-3">
                  <Text className="text-xs" style={{ color: colors.primary }}>
                    #{item.ticketId}
                  </Text>
                  <Text className="text-base font-semibold mt-1" style={{ color: colors.textPrimary }}>
                    {item.title}
                  </Text>
                  <Text className="text-xs mt-1" style={{ color: colors.textSecondary }}>
                    by {item.citizenName}
              ...
              {item.comment ? (
                ...
                <Text className="text-sm italic" style={{ color: colors.textSecondary }}>
                  No written feedback left for this complaint.
                </Text>
              )}
            </Card>
          ))
