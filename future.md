# Sahayak Deep Audit, Roadmap, And Expansion Plan

This document is the product and engineering audit after a full backend/frontend scan of the current repository.

It answers four questions:

1. What is duplicated in the backend today?
2. What does the backend already support that the frontend still does not expose well?
3. What concrete steps should we take next to improve architecture and product quality?
4. What higher-value features can we add on top of the current foundation?

The goal is not to produce a vague wishlist. The goal is to turn the current codebase into a cleaner, more complete product with less repeated logic and stronger frontend coverage.
...
### 3.5 Notification Preference UX

Backend support:

- notification preference endpoints
- reusable mobile hooks already exist

Frontend status:

- backend and hooks are ready
- end-user settings UX is still weak or absent

Improvement steps:

1. Add notification preference screen under `More`.
2. Split toggles by:
   - complaint updates
   - assignments
   - escalations
   - system alerts
3. Show role-aware disabled states where a preference is irrelevant.

### 3.6 Report Stats And Department Breakdown Are Backend-Ready But Not Always Surfaced
...
- the current HOD reports screen intentionally removed the analytics block to simplify UX

Improvement steps:

1. Reintroduce analytics only when the reports IA is redesigned.
2. Keep export/schedule flows separate from analytics insights.
3. Consider a dedicated operations insights screen instead of cramming everything into reports.

### 3.7 Admin Operations And Delivery Health

Backend support already exists indirectly through:

- report schedule health fields
...
- there is no admin-facing operations console

Improvement steps:

1. Add admin operations dashboard for:
   - failed report schedules
   - notification delivery trends
   - unresolved escalations
   - stale complaints
2. Add run-now retry, inspection, and failure drill-down.

---

## 4. Frontend Duplication And Hardening Audit

These are the highest-signal mobile areas still carrying too much local orchestration.

### 4.1 Screens Still Owning Raw `apiCall(...)` Logic

High-signal current screens:
...
- multiple auth screens

Why this matters:

- refresh behavior differs by screen
- loading/error/empty states drift
- cache invalidation becomes less reliable

Improvement steps:

1. Finish moving list and detail screens onto domain hooks.
2. Keep filters and pagination serialization inside hook layers.
3. Return stable view models from hooks instead of reshaping data in screens.

### 4.2 Dashboard Screens Still Mix Data Concerns With Presentation Concerns
...
1. Move card/view-model shaping into hooks.
2. Keep screens focused on rendering.
3. Use shared dashboard card primitives for consistency.

### 4.3 Auth Lifecycle Still Lives Across Several Utilities And Screens
...
1. Create auth domain hooks.
2. Centralize post-login/post-refresh side effects.
3. Publish one auth lifecycle diagram in README.
